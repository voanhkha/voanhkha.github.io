...
+ If at any time step \\(t_k\\) whenever there is no food remaining, or equivalently saying that the agent just collects the last food at \\(t = k \\), then, \\(r_k = +1 \\), and terminate the game.
